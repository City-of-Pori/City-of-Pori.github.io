...
As the Chromebook automation couldn't be made fool-proof (see Post-mortem), a decision was made to insert pauses in the script sending the keypresses to the devices. A USB presenter connected to the laptop was used to resume the script remotely.

While the solution of using the KVM-switches worked, see Arduinos all the way in the Post-mortem section for a discussion on a better alternative.
...
This would have also enabled circumventing the Simultaneous enrollment limits problem as a random delay could have been added on a per device basis. Also the KVM-switches were somewhat slow in moving in and out of the broadcast mode as well as in switching between individual ports.
